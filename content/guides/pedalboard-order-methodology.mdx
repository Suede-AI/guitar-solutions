...
The canonical signal chain map, dynamics, drive, modulation, time, and amp, is a starting point, not a law. Understanding why the order exists is what tells you when to follow it exactly and when to break it with intention.
...
The canonical order describes what sounds best to most guitarists most of the time. It is a description of observed results, not a specification handed down. The reason to learn it is that understanding the cause lets you deviate with purpose.

## Dynamics first: compressor and wah

**Compressor:** Before drive, the compressor works on your actual picking dynamics. Heavy attack gets clamped; light playing gets lifted. The drive stage then receives a more consistent level, which makes it tighter and more controlled. A compressor placed after drive compresses an already-saturated signal. This is useful for peak control (taming spikes going into a power amp or PA), but it does not shape playing feel the way front-of-amp compression does.

**Wah:** Before drive produces the classic rock and funk wah sound. The filter sweeps an unprocessed guitar signal, then the drive colors the swept signal. After drive, the wah sweeps an already-saturated signal. The resonant peak is smoother, less sharp, and more modern. Both positions produce working sounds. Know which one you want.

## Drive in the middle

Overdrive, distortion, and fuzz usually belong after initial dynamics shaping and before time-based effects. The exception is placing a drive pedal in an amp's effects loop, covered separately in the effects loop guide.

**Germanium fuzz:** Before the first buffer and before most other pedals. The fuzz input circuit uses your pickup's impedance as part of its tone network. Buffering before a germanium fuzz changes the cleanup and input response.

**Silicon fuzz and modern distortion:** Less impedance-sensitive. More flexible positioning.

**Stacking drives:** The first drive in the chain sets the character the second drive amplifies and reacts to. A mild overdrive feeding a distortion sounds different from the same distortion feeding the same overdrive. There is no rule for stacking order. Test the order deliberately.
...
**After drive:** The shifter attempts to track a harmonically complex, saturated signal. Tracking degrades, with more tracking errors, less pitch accuracy, and worse results on chords.
...
**Before drives:** Controls the level entering your gain stage. Backing off cleans up the drive because the pedal or amp responds to a lower input level with less saturation. This mimics rolling back your guitar's volume knob.
...
Play the same bar through two orders. Keep every knob still except the order.
...
| Pass | Order | What changes |
|---|---|---|
| A | Compressor, drive, delay | Pick dynamics tighten before the gain stage |
| B | Drive, compressor, delay | Sustain rises, but the drive texture flattens |
| C | Drive, delay, reverb | Repeats stay distinct before the reverb tail |
| D | Delay, drive, reverb | Repeats saturate and blur into the gain |

## Quick Diagnosis

| Symptom | Likely cause | First check | Fix |
|---|---|---|---|
| Fuzz feels stiff | Buffer or pedal before a germanium fuzz | Put fuzz first for one pass | Keep impedance-sensitive fuzz at the front |
| Modulation disappears | Chorus or phaser is before heavy gain | Move modulation after drive | Modulate the finished drive tone |
| Pitch pedal glitches on chords | Tracker is after distortion | Move pitch before drive | Feed the pitch detector a cleaner signal |

## Further Reading

See also: [Signal Chain Topology](/guides/signal-chain-topology) for the full five-block map. [The Effects Loop](/guides/effects-loop-serial-vs-parallel) for routing time effects after preamp gain.
