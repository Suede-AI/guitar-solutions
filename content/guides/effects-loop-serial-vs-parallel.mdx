...
A pedal placed in the loop processes the sound of your preamp, not the raw guitar. The tone shape, the gain character, and the amp EQ all enter the loop pedal's input. The loop pedal's output feeds the power amp directly.

## Why this matters for time-based effects

Delay and reverb produce repeats and reflections that sound most natural when they're applied to an already-formed tone. If you run delay into the front of a driven amp, the amp's preamp processes both the original note and every one of its repeats. Each repeat is re-driven, re-compressed, and re-shaped. The delays become thick and indistinct.
...
A **parallel loop**, less common and found on some boutique and high-end amps, splits the signal into two paths: one through the loop pedal, one bypassing it. The amp blends them before the power amp stage using a Mix control. At 100% wet, you hear only the loop pedal. At 0%, you hear the dry amp path. Between them, you get both.

The parallel loop is useful for reverb and delay because it preserves the amp's dry signal on one path while adding depth on the other. Some engineers prefer this because the dry path never touches a converter or a buffer.

The tradeoff: the parallel loop's Mix control behaves poorly with drive pedals. You cannot blend a processed and an unprocessed signal cleanly when the processed signal contains heavy harmonic content.

## The four-cable method

The four-cable method (4CM) routes a multi-effects unit or switcher so that some effects sit before the amp's preamp and others sit in the effects loop. The physical amp remains the gain stage in the middle.
...
- **Delay, reverb, chorus, flanger, phaser:** Start in the effects loop when the amp preamp supplies most of the gain.
...
## Try This: Front vs. Loop Delay

Use a short palm-muted phrase so the repeats are easy to hear.
...
| Pass | Placement | Settings | Expected difference |
|---|---|---|---|
| A | Delay before amp input | 350 ms, 25% mix, 3 repeats | Repeats hit the preamp and smear with gain |
| B | Delay in the loop | Same settings | Repeats stay clearer because they bypass preamp distortion |
| C | Reverb in the loop after delay | Small room, low mix | The repeat trail sits behind the driven tone |

## Quick Diagnosis

| Symptom | Likely cause | First check | Fix |
|---|---|---|---|
| Delay repeats turn to mush | Delay is before a driven preamp | Move the delay into the loop for one pass | Keep time effects after the main gain stage |
| Loop sounds thin or noisy | Level mismatch | Check -10 dBV vs +4 dBu on amp and pedal | Match levels or add a converter |
| Parallel loop has phasey repeats | Wet/dry blend conflict | Set the pedal fully wet if possible | Let the amp handle the dry blend |

## Further Reading

See also: [Pedalboard Order](/guides/pedalboard-order-methodology) for what belongs before the amp input. [Signal Chain Topology](/guides/signal-chain-topology) for where the loop sits in the full chain.
